 ...
 */
class Asqlite {
  static TABLE_NAME = "LMIAs";
  /**
   * Create a new {@link Asqlite} from a {@link sqlite3.Database} object.
   *
   * @see Use {@link Asqlite.open} to create and open the database with the async API.
   *
  ...
  /**
   * Returns a new {@link Asqlite} object and automatically opens the database.
   *
  ...
  static open(
    filename: string,
    mode: number = sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE | sqlite3.OPEN_CREATE
  ): Promise<Asqlite> {
    return new Promise((resolve, reject) => {
      const db = new sqlite3.Database(filename, mode, (err) => {
        if (err) reject(err);
        else resolve(new Asqlite(db));
      });
    });
  }
